...
// --------------------------------------------------------------------------------------
// simRTDE.connect: Connect to URSim
// --------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------
// simRTDE.disconnect: Disconnect from URSim
// --------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------
// simRTDE.getIO(): Get IO
// --------------------------------------------------------------------------------------
#define LUA_GET_IO_COMMAND "simRTDE.getIO" // the name of the new Lua command

void LUA_GET_IO_CALLBACK(SScriptCallBack *p)
{
  // the callback function of the new Lua command ("simRTDE.disconnect")
  int stack = p->stackID;
  ...
  if (inArguments.getSize() == 1 && inArguments.isNumber(0))
  {
    if (plugin_loaded)
    {
      ...
      bool signal = ptr_rtde_client->getDigitalIO(io_id);
...
// --------------------------------------------------------------------------------------
// simRTDE.setIO(): Set IO
// --------------------------------------------------------------------------------------
#define LUA_SET_IO_COMMAND "simRTDE.setIO" // the name of the new Lua command

void LUA_SET_IO_CALLBACK(SScriptCallBack *p)
{
  // the callback function of the new Lua command ("simRTDE.disconnect")
  int stack = p->stackID;
  ...
  if (inArguments.getSize() == 2 && inArguments.isNumber(0) && inArguments.isBool(1))
  {
    if (plugin_loaded)
    {
      int io_id = inArguments.getInt(0);
...
SIM_DLLEXPORT unsigned char simStart(void *reservedPointer, int reservedInt)
{
  ...
  simRegisterScriptCallbackFunction(strConCat(LUA_GET_IO_COMMAND, "@", "RTDE"), strConCat("bool signal=", LUA_GET_IO_COMMAND, "(number IO_id)"), LUA_GET_IO_CALLBACK);
  simRegisterScriptCallbackFunction(strConCat(LUA_SET_IO_COMMAND, "@", "RTDE"), strConCat("...=", LUA_SET_IO_COMMAND, "(number IO_id, bool IO_signal)"), LUA_SET_IO_CALLBACK);
